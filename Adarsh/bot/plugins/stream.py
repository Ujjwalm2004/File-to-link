...
from Adarsh.utils.shortner import get_shortlink
...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        stream_link = await get_shortlink(f"{Var.URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}", u_id=m.from_user.id)
        
        online_link = await get_shortlink(f"{Var.URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}", u_id=m.from_user.id)
